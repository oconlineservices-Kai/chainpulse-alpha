...
export const GET = auth(async (req) => {
  ...
  } catch (e: any) { results.userCountError = e.message }
  
  try {
    results.signalCount = await prisma.signal.count()
  } catch (e: any) { results.signalCountError = e.message }
  
  try {
    results.waitlistCount = await prisma.waitlist.count()
  } catch (e: any) { results.waitlistCountError = e.message }
  
  try {
    results.transactionCount = await prisma.transaction.count({ where: { status: 'completed' } })
  } catch (e: any) { results.transactionCountError = e.message }
  
  try {
    results.premiumUsersCount = await prisma.user.count({ where: { premiumStatus: { not: 'free' } } })
  } catch (e: any) { results.premiumUsersError = e.message }
  
  try {
    results.adminUsersCount = await prisma.user.count({ where: { premiumStatus: 'admin' } })
  } catch (e: any) { results.adminUsersError = e.message }
  
  try {
    results.recentSignals = await prisma.signal.findMany({
      orderBy: { createdAt: 'desc' },
      take: 3,
      select: { tokenSymbol: true, sentimentScore: true, priceChangePct: true, performanceStatus: true, createdAt: true }
    })
  } catch (e: any) { results.recentSignalsError = e.message }
  ...
  } catch (e: any) { results.totalRevenueError = e.message }
  ...
  } catch (e: any) { results.activeUsersError = e.message }
